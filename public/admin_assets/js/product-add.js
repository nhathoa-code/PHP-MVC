...
$("#add-product-form").submit(function (e) {
  ...
  $.ajax({
    ...
    error: function (xhr, status, error) {
      ...
      let error_res = xhr.responseJSON;
      let str = error_res.message;
      if (error_res.hasOwnProperty("error_bag")) {
        let errors = JSON.parse(error_res.error_bag);
        for (let item in errors) {
          str += `${item} : ${errors[item].join(",")}`;
        }
      }
      notif({
        msg: str,
...
$(document).on(
  "input",
  "#add-product-form input[name=p_price], #add-product-form input.inventory, #add-product-form input.price, #add-product-form #type-all input",
  function () {
    $(this).mask("0#");
    let value = $(this).val();
    if (/^0+[1-9]/.test(value)) {
