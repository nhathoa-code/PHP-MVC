!(function (a, b, c, d) {
  ...
  function m(b) {
    return a(
      '<span id="MonthPicker_Button_' +
        this.id +
        '" class="month-picker-open-button">' +
        b.i18n.buttonText +
        "</span>"
    ).jqueryUIButton({ text: !1, icons: { primary: b.ButtonIcon } });
  }
  function n(a, b) {
    a.jqueryUIButton("option", {
      icons: { primary: "ui-icon-circle-triangle-" + (b ? "w" : "e") },
    });
  }
  function o(a) {
    return !a.is("input");
  ...
  function p(b, c) {
    ...
    function e() {
      (j = null), (l = a("span", b).animate({ opacity: 0.45 }, k, f));
    }
    function f() {
      (i = l.text()), l.animate({ opacity: 1 }, k).text(c);
    }
    function g() {
      j ? clearTimeout(j) : (l = a("span", b).animate({ opacity: 0.45 }, k, h));
    }
    function h() {
  ...
  if (!(a && a.ui && a.ui.button && a.ui.datepicker))
    return void alert(
      r + "The jQuery UI button and datepicker plug-ins must be loaded."
    );
  ...
  var O =
    '<div class="ui-widget-header month-picker-header ui-corner-all"><table class="month-picker-year-table"><tr><td class="month-picker-previous"><a /></td><td class="month-picker-title"><a /></td><td class="month-picker-next"><a /></td></tr></table></div><div><table class="month-picker-month-table" /></div>';
  a.widget("KidSysco.MonthPicker", {
    ...
    _destroy: function () {
      var b = this.element;
      a.mask &&
        this.options.UseInputMask &&
        (b.unmask(), this.GetSelectedDate() || b.val("")),
        b.removeClass(u).off(t),
        a(c).off(t + this.uuid),
        this._monthPickerMenu.remove();
      var d = this._monthPickerButton.off(N);
      this._removeOldBtn && d.remove(),
    ...
    _setOption: function (b, c) {
      ...
      return b in I && -1 === a.inArray(c, I[b])
        ? void alert(E.replace(/%/, b) + I[b])
        : (this._super(b, c), void (J[b] ? this[J[b]](c) : 0));
    },
    _create: function () {
      var b = this.element,
        e = this.options,
      ...
      if (!a.mask && e.UseInputMask)
        return (
          alert(
            r +
              "The UseInputMask option requires the Input Mask Plugin. Get it from digitalbush.com"
          ),
          !1
        );
      if (null !== e.Position && !H) return alert(D), !1;
      for (var i in I)
        if (null !== e[i] && -1 === a.inArray(e[i], I[i]))
          return alert(E.replace(/%/, i) + I[i]), !1;
      (this._isMonthInputType = "month" === b.attr("type")),
        this._isMonthInputType &&
          ((this.options.MonthFormat = this.MonthInputFormat),
          b.css("width", "auto"));
      var k = (this._monthPickerMenu = a(
          '<div id="MonthPicker_' +
            b[0].id +
            '" class="month-picker ui-widget ui-widget-content ui-corner-all"></div>'
        ).hide()),
        l = o(b);
      a(O).appendTo(k),
        k.appendTo(l ? b : c.body),
        (this._titleButton = a(".month-picker-title", k)
          .click(L(this._showYearsClickHandler, this))
          .find("a")
          .jqueryUIButton()
          .removeClass(A)),
        this._applyJumpYearsHint(),
        this._createValidationMessage(),
        (this._prevButton = a(".month-picker-previous>a", k)
          .jqueryUIButton({ text: !1 })
          .removeClass(A)),
        (this._nextButton = a(".month-picker-next>a", k)
          .jqueryUIButton({ text: !1 })
          .removeClass(A)),
        this._setRTL(e.IsRTL),
        a(w, this._nextButton).text(this._i18n("nextLabel")),
      ...
      (this._buttons = a("a", m).jqueryUIButton()),
        k.on("mousedown" + t, function (a) {
          a.preventDefault();
        });
      var q = this,
        s = "Month";
      ...
      this._updateAlt(),
        ...
        l ? this.Open() : (b.addClass(u), b.change(L(this._updateAlt, this)));
    },
    ...
    Validate: function () {
      ...
      return (
        null === this.options.ValidationErrorMessage ||
          ...
          this._validationMessage.toggle(!a),
        a
      );
    },
    ...
    Open: function (b) {
      ...
      if (!e.Disabled && !this._visible) {
        if (
          ((b = b || a.Event()),
          k("OnBeforeMenuOpen", this)(b) === !1 || b.isDefaultPrevented())
        )
          return;
        (this._visible = !0), this._ajustYear(e);
        var f = this._monthPickerMenu;
        if ((this._showMonths(), o(d)))
          f.css("position", "static").show(), k("OnAfterMenuOpen", this)();
        else {
          G && G.Close(b),
            (G = this),
            a(c)
              .on("mousedown" + t + this.uuid, L(this.Close, this))
              .on("keydown" + t + this.uuid, L(this._keyDown, this)),
            d.off("blur" + t).focus();
    ...
    Close: function (b) {
      ...
      if (!o(d) && this._visible) {
        ...
        if (
          ((b = b || a.Event()),
          k("OnBeforeMenuClose", this)(b) === !1 || b.isDefaultPrevented())
        )
          return;
        this._backToYear &&
          (this._applyJumpYearsHint(), (this._backToYear = 0)),
          (this._visible = !1),
          (G = null),
          a(c)
            .off("keydown" + t + this.uuid)
            .off("mousedown" + t + this.uuid),
          this.Validate(),
          d.on("blur" + t, L(this.Validate, this));
    ...
    _createButton: function () {
      ...
      if (!o(b)) {
        ...
        var h = !1;
        (this._monthPickerButton = (f instanceof a ? f : a(f))
          .each(function () {
            a.contains(c.body, this) || ((h = !0), a(this).insertAfter(b));
          })
          .on(N, L(this.Toggle, this))
          .on("mousedown" + t, function (a) {
            a.preventDefault();
          })),
          this._removeOldBtn && e.remove(),
          (this._removeOldBtn = h);
      }
    },
    _updateFieldEvents: function () {
      var a = N + " focus" + t;
      this.element.off(a),
        ("both" !== this.options.ShowOn && this._monthPickerButton.length) ||
          this.element.on(a, L(this.Open, this));
    },
    _createValidationMessage: function () {
      ...
      if (-1 === a.inArray(b, [null, ""])) {
        var d = a(
            '<span id="MonthPicker_Validation_' +
              c[0].id +
              '" class="month-picker-invalid-message">' +
              b +
              "</span>"
          ),
    ...
    _setRTL: function (a) {
      n(this._prevButton.css("float", a ? "right" : "left"), !a),
        n(this._nextButton.css("float", a ? "left" : "right"), a);
    },
    _keyDown: function (a) {
      switch (a.keyCode) {
        case 13:
          this.element.val() || this._chooseMonth(new d().getMonth() + 1),
            this.Close(a);
          break;
        case 27:
        case 9:
    ...
    _position: H
      ...
      : function (a) {
          ...
          return (
            this.options.IsRTL
              ? (c.left = b.offset().left - a.width() + b.width() + 7 + "px")
              : (c.left = b.offset().left + "px"),
            a.css(c)
          );
        },
    _setUseInputMask: function () {
      if (!this._isMonthInputType)
        try {
          this.options.UseInputMask
            ? this.element.mask(this._formatMonth(new d()).replace(/\d/g, 9))
            : this.element.unmask();
        } catch (a) {}
    },
    ...
    _setPickerYear: function (a) {
      (this._pickerYear = a || new d().getFullYear()),
    ...
    _chooseMonth: function (b) {
      ...
      this.element.val(this._formatMonth(e)).blur(),
        ...
        (this._selectedBtn = h(a(this._buttons[b - 1]), !0)),
        k("OnAfterChooseMonth", this)(e);
    },
    _chooseYear: function (a) {
      (this._backToYear = 0),
        this._setPickerYear(a),
        this._buttons.removeClass(y),
        this._showMonths(),
        this._applyJumpYearsHint(),
        k("OnAfterChooseYear", this)();
    },
    _showMonths: function () {
      ...
      a.each(b, function (b, e) {
        a(c._buttons[b])
          .on(N, { month: b + 1 }, d)
          .jqueryUIButton("option", "label", e);
      }),
        this._decorateButtons();
    },
    _showYearsClickHandler: function () {
      if ((this._buttons.removeClass(y), this._backToYear))
        ...
        this._titleButton.jqueryUIButton({ label: a }).data(v)(),
          k("OnAfterChooseYears", this)();
      }
    },
    _showYears: function () {
      ...
      this._prevButton
        ...
        .off(N)
        .on(N, L(this._addToYears, this, -g)),
        this._nextButton
      ...
      for (
        ...
        var v = b + c,
          w = a(this._buttons[u])
            .jqueryUIButton({ disabled: !i(v, m, n), label: v })
            .toggleClass(y, v === j && r)
            .on(N, { year: v }, p);
    ...
    _addToYears: function (a) {
      (this._pickerYear = this._getPickerYear() + a),
        this._showYears(),
        this.element.focus(),
    ...
    _decorateButtons: function () {
      ...
      k &&
        k.getFullYear() === b &&
        (this._selectedBtn = h(a(this._buttons[k.getMonth()]), l)),
